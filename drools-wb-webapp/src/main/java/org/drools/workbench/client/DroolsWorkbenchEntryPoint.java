...
public class DroolsWorkbenchEntryPoint {
	...
	private void loadPreferences() {
		UberFirePreferences
				.setProperty(
						"org.uberfire.client.workbench.widgets.listbar.context.disable",
						true);
	...
	private void setupMenu() {
		...
		final Menus menus = MenuFactory
				...
				.respondsWith(new Command() {
					...
					public void execute() {
						if (defaultPerspective != null) {
							placeManager.goTo(new DefaultPlaceRequest(
									defaultPerspective.getIdentifier()));
						} else {
				...
				.newTopLevelCustomMenu(
						iocManager.lookupBean(CustomSplashHelp.class)
								.getInstance()).endMenu().build();
	...
	private List<MenuItem> getPerspectives() {
		...
		for (final AbstractWorkbenchPerspectiveActivity perspective : getPerspectiveActivities()) {
			...
			final Command cmd = new Command() {
				...
				@Override
				public void execute() {
					placeManager.goTo(new DefaultPlaceRequest(perspective
							.getIdentifier()));
				}

			};
			final MenuItem item = newSimpleItem(name).respondsWith(cmd)
	...
	private AbstractWorkbenchPerspectiveActivity getDefaultPerspectiveActivity() {
		...
		final Iterator<IOCBeanDef<AbstractWorkbenchPerspectiveActivity>> perspectivesIterator = perspectives
				.iterator();
		outer_loop: while (perspectivesIterator.hasNext()) {
			final IOCBeanDef<AbstractWorkbenchPerspectiveActivity> perspective = perspectivesIterator
					.next();
			final AbstractWorkbenchPerspectiveActivity instance = perspective
					.getInstance();
		...
		Collections.sort(sortedActivities,
				new Comparator<AbstractWorkbenchPerspectiveActivity>() {
					...
					public int compare(AbstractWorkbenchPerspectiveActivity o1,
						...
						return o1.getPerspective().getName()
								.compareTo(o2.getPerspective().getName());
					}
